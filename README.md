...
### Отладка backend (Cursor / VS Code)

В проекте настроена конфигурация отладки Node.js (`.vscode/launch.json`). Запуск сервера под отладчиком:

1. Откройте панель **Run and Debug**: `Cmd+Shift+D` (macOS) или `Ctrl+Shift+D` (Windows/Linux).
2. Выберите конфигурацию **«Debug Server (dev)»**.
3. Нажмите **F5** или зелёную кнопку **Start Debugging**.
4. Ставьте breakpoint'ы в коде (клик слева от номера строки в редакторе).

Фронтенд в dev при этом запускайте отдельно в терминале: `npm run frontend-dev`.

**Подсказка (macOS):** можно вызвать команду из корня проекта — `npm run dev:debug` — чтобы вывести напоминание по горячим клавишам.
...
Для полного описания всех npm скриптов см. [SCRIPTS.md](./SCRIPTS.md). Конфигурация отладки backend: `.vscode/launch.json` (конфиг **Debug Server (dev)**).
